...
from class_NES_two import NES
from config_two import CLASS_NUM, OUT_DIR
...
def get_args():
    ...
    parser.add_argument('--exp-dir', default='/data/output_allHigh_v3/data_cvcN_nes02/', type=str,
                        help='directory to save results and logs')
    parser.add_argument('--data', type=str, default='/root/myProject/HOTCOLDBlock-main-V3/data/input_v3/dataCvc09AllHighPre/nighttime/nighttime.yaml', help='dataset.yaml path')
    parser.add_argument('--dataset_name', type=str, default='CVC09-N', help='dataset.yaml path')
...
if __name__ == "__main__":
    ...
    #加载模型
    hyp = 'target_model/yolov3/runs/train/exp2-CvcN/hyp.yaml'
    if isinstance(hyp, str):
        with open(hyp, errors='ignore') as f:  # f中包含了配置文件.yaml中的所有内容
            hyp = yaml.safe_load(f)  # hyp返回python字典，通过get()方法调取其中的参数
    # coco数据集上的预训练权重
    # weights = "runs/train/exp/weights/best.pt"
    weights = "target_model/yolov3/runs/train/exp2-CvcN/weights/best.pt"
    ckpt = torch.load(weights, map_location='cpu')
    model = Model(ckpt['model'].yaml, ch=3, nc=1, anchors=hyp.get('anchors')).to(device)
