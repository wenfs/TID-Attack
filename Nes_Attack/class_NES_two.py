...
class NES(object):
    ...
    def attack_all_images(self, args, arch_name, target_model, result_dump_path):
        ...
        # 保存stats信息
        data = np.save('/root/myProject/HOTCOLDBlock-main-V3/data/output_allHigh_v3/data_cvcN_nes02/stats_ori_info', bb)
        ...
        data_adv = np.save('/root/myProject/HOTCOLDBlock-main-V3/data/output_allHigh_v3/data_cvcN_nes02/stats_adv_info', aa)
    ...
    def make_adversarial_examples(self, batch_index, images, args, targets, target_model, shapes, stats_adv_simba_flir, seen_adv):
        ...
        correct = True if after_selectConf_out >= 0.35 else False
        ...
        while query[0].item() < args.max_queries:
            ...
            not_done = True if after_selectConf_out >= 0.35 else False
...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=True, save_dir='.', OUT_DIR = ".", names=(), eps=1e-16, tag = True):
    ...
    if plot:
        ...
        if tag:
            plot_pr_curve(px, py, ap, '/data/output_allHigh_v3/data_cvcN_nes02/PR_ori_curve.png', names)
        else:
            plot_pr_curve(px, py, ap, '/data/output_allHigh_v3/data_cvcN_nes02/PR_adv_curve.png', names)
