...
class ZOOAttack(object):
    def __init__(self, model, num_channels, img_size, args, log):
        ...
        #######################################
        self.myloss = ComputeLoss(self.model)
        if self.use_tanh:
    ...
    def loss(self, newimg, timg, targets, const):
        with torch.no_grad():
            logits, logits_out = self.model(newimg.half())  # B, # num_class
        if self.use_log:
        ...
        loss1_val= self.myloss(logits_out, targets)
    ...
    def blackbox_optimizer(self, timg, targets, const):
        ...
        losses, loss1, loss2, scores = self.loss(newimg, timg, targets, const)
    ...
    def attack(self, img, targets, log, batch_index):
        batch_size = img.size(0)
        query = torch.zeros(batch_size).cuda()
        with torch.no_grad():
            pred, pred_out = self.model(img)  # inference, loss outputs
        ...
        correct = True if after_selectConf_out >= 0.5 else False
        ...
        adv_loss = self.myloss(pred_out, targets)
        ...
        if correct:
            for outer_step in range(self.BINARY_SEARCH_STEPS):
                ...
                for iteration in range(self.start_iter, self.MAX_ITERATIONS):
                    ...
                    # else:
                    #
                    l, l2, loss1, loss2, score, nimg = self.blackbox_optimizer(timg, targets, const)
                    ...
                    if l2 < o_bestl2:
                        ...
                        with torch.no_grad():
                            adv_logit,adv_logit_out = self.model(o_bestattack.half())
                        # adv_prob = F.softmax(adv_logit, dim=1)
                        # adv_pred = adv_logit.argmax(dim=1)
                        lb = []
                        adv_out = non_max_suppression(adv_logit, conf_thres, iou_thres, labels=lb, multi_label=True,
                                                      agnostic=single_cls)[0]
                        adv_conf = adv_out[:, 4]
                        after_selectConf_out = np.mean(adv_conf.detach().cpu().numpy())
                        adv_loss = self.myloss(adv_logit_out, targets)

                        log.print( "iter:{},  cur_loss:{},  cur_confidence:{},  cur_query:{}".format(iteration, adv_loss.item(), after_selectConf_out,
                                                                                              eval_costs))

                        not_done = True if after_selectConf_out >= 0.5 else False
                        ...
                        if self.ABORT_EARLY:
                            if loss2 <= self.epsilone and not not_done.byte().any(): #攻击成功+l2距离低于预设阈值
                                adv_images_founded = True
                                log.print("Early stopping attack successfully and total pixels' distortion is {:.3f}".format(loss2))
                                break
                            # prev = l
                    train_timer += time.time() - attack_begin_time

                if not_done and bestscore!=-1:
            ...
            log.print("image {} is already adversarial".format(batch_index))
