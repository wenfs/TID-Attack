...
def proj(adv_imgs, orig_imgs, diff, max_distance):
    if adv_imgs.dim() == 3:
        adv_imgs = adv_imgs.unsqueeze(0)
        orig_imgs = orig_imgs.unsqueeze(0)
        diff = diff.unsqueeze(0)
    diff_norm = distance(torch.clamp(adv_imgs + diff, 0, 1), orig_imgs)
...
def selectConfPro(out, pre_conf):
    output = []
    for si in range(out.shape[0]):
    #根据置信度分数筛选bbox，>0.5的
        if pre_conf[si] > 0.5:
            output.append(out[si])
            #可视化最后留下的bounding box，label表明类别和置信度分数
    return output
